...
while rodando:
    ...
    if vg.MENU_MELHORIA:
        ...
        if pause:
            ...
            if utils.criar_botao_imagem("Continuar", vg.botao_folder, vg.LARGURA//2, 400):
                pause = not pause

        if gamer_over:

            overlay = pg.Surface((vg.LARGURA, vg.ALTURA))
            overlay.set_alpha(200)
            overlay.fill(CINZA)

            vg.tela.blit(overlay, (0, 0))

            vg.tela.blit(texto_morte, (325, 50))
